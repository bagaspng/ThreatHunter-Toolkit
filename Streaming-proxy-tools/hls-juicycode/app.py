...
from juicy_decoder import JuicyDecoder
    ...
    <body>
        ...
        <div class="container">
            ...
            <div class="card input-section">
                <h3 style="font-size: 15px; font-weight: 600;">Extractor & Player</h3>
                <div class="form-group" style="border-bottom: 1px solid var(--border-color); padding-bottom: 16px; margin-bottom: 6px;">
                    <input type="text" id="iframeInput" placeholder="Masukkan URL Iframe Asli (Contoh: https://199.87.210.226/player/...)" value="https://199.87.210.226/player/1jAfXErgmMlBbiZ">
                    <button class="btn-extract" onclick="extractStream()">Extract M3U8</button>
                </div>
                <div class="form-group">
                    <input type="text" id="urlInput" placeholder="URL M3U8 Hasil Ekstraksi akan muncul di sini..." value="">
        ...
        <script>
            ...
            async function extractStream() {
                var iframeUrl = document.getElementById('iframeInput').value.trim();
                if (!iframeUrl) {
                    log('Error: URL Iframe kosong!', 'error');
                    return;
                }

                log(`Memulai proses ekstraksi dari: <span class="url-text">${iframeUrl}</span>`, 'info');
                showLoader(true, 'Extracting payload...');
                
                try {
                    const response = await fetch('/api/extract?url=' + encodeURIComponent(iframeUrl));
                    const data = await response.json();
                    
                    if (data.status === 'success') {
                        log('Berhasil mendekripsi payload JuicyCodes!', 'success');
                        log(`Ditemukan M3U8 URL: <span class="url-text">${data.m3u8_url}</span>`, 'success');
                        
                        document.getElementById('urlInput').value = data.m3u8_url;
                        showLoader(false);
                        
                        // Auto-play the extracted URL
                        loadStream();
                    } else {
                        log(`Gagal mengekstrak: ${data.message}`, 'error');
                        showLoader(false);
                    }
                } catch (err) {
...
def extract_juicycodes():
    iframe_url = request.args.get("url")
    if not iframe_url:
        return jsonify({"status": "error", "message": "Missing url parameter"}), 400
        
    result = JuicyDecoder.run_pipeline(iframe_url)
