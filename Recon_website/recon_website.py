...
PHONE_REGEX = re.compile(
    "(?:\\+62|62|0)8[1-9][0-9]{6,10}|(?:\\+62|62|0)[2-9][0-9]{1,2}[\\s\\-]?[0-9]{3,4}[\\s\\-]?[0-9]{3,4}"
)
EMAIL_OBFUSCATED_REGEX = re.compile(
    "[a-zA-Z0-9._%+\\-]+\\s*[\\[\\(]\\s*at\\s*[\\]\\)]\\s*[a-zA-Z0-9.\\-]+\\s*[\\[\\(]\\s*dot\\s*[\\]\\)]\\s*[a-zA-Z]{2,}(?:\\s*[\\[\\(]\\s*dot\\s*[\\]\\)]\\s*[a-zA-Z]{2,})*",
    re.IGNORECASE,
)
try:
...
def fetch(url):
    try:
        ...
        try:
            resp = requests.get(
                url, headers=random_headers(), timeout=TIMEOUT, verify=False
            )
            resp.raise_for_status()
            soup = BeautifulSoup(resp.text, "html.parser")
            ssl_warning = (
                f"Diakses TANPA verifikasi SSL karena sertifikat bermasalah: {e}"
            )
...
def discover_paths(base_url, base_soup, log_fn, max_paths=MAX_PATHS_PER_DOMAIN):
    ...
    for a in base_soup.find_all("a", href=True):
        ...
        if len(discovered) >= max_paths:
            break
    log_fn(
        f"  [i] Ditemukan {len(discovered)} path internal via BeautifulSoup (maks {max_paths})."
    )
    return discovered
...
def nmap_scan(domain):
    if not HAS_NMAP:
        return (
            None,
            "nmap tidak ditemukan di sistem (pastikan sudah terinstall dan ada di PATH)",
        )
    try:
        result = subprocess.run(
            ["nmap", "-F", "-T4", domain], capture_output=True, text=True, timeout=120
        )
            ...
            with ctx.wrap_socket(sock, server_hostname=hostname) as tls_sock:
                ...
                if der_cert:
                    try:
                        ...
                        issuer_org = cert_obj.issuer.get_attributes_for_oid(
                            x509.oid.NameOID.ORGANIZATION_NAME
                        )
                        info["cert_issuer"] = (
                            issuer_org[0].value
                            if issuer_org
                            else issuer_cn[0].value if issuer_cn else None
                        )
                        not_after_dt = (
                            getattr(cert_obj, "not_valid_after_utc", None)
                            or cert_obj.not_valid_after
                        )
                        info["cert_not_after"] = not_after_dt.strftime(
                            "%Y-%m-%d %H:%M:%S"
                        )
                        now_ref = datetime.utcnow()
                        if not_after_dt.tzinfo is not None:
                        ...
                        info["error"] = (
                            "Library 'cryptography' tidak terinstall, tidak bisa parse detail sertifikat (pip install cryptography)."
                        )
...
def recon_single_target(base_url, log_fn):
    ...
    if not resp:
        msg = f"Gagal mengakses {base_url} -> {err}"
        log_fn(f"  [!] {msg}")
    ...
    for path_url in internal_paths:
        ...
        if not p_resp:
            log_fn(f"  [!] Gagal mengakses {path_url} -> {p_err}")
            continue
        if p_ssl_warning:
    ...
    log_fn(f"\n--- Tahap 4: Cek TLS/SSL & kecepatan koneksi pada {domain_only} ---")
    ...
    if ssl_info["error"]:
        ...
        log_fn(
            f"  [+] Sertifikat expired: {ssl_info['cert_not_after']}{(' (SUDAH EXPIRED)' if ssl_info['cert_expired'] else '')}"
        )
        if ssl_info["cert_expired"]:
            result["errors"].append(
                f"Sertifikat SSL sudah expired sejak {ssl_info['cert_not_after']}"
            )
    ...
    if speed_info["error"]:
        ...
        log_fn(
            f"  [+] Waktu response HTTP penuh: {speed_info['total_ms']} ms (status {speed_info['status_code']})"
        )
...
def main():
    if len(sys.argv) < 2:
        print("Cara pakai:")
        print("  python recon_website.py daftar_url.txt")
        print("")
