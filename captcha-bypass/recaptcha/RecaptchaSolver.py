...
class RecaptchaSolver:
    ...
    def solveCaptcha(self) -> None:
        ...
        for attempt in range(5):
            try:
                ...
                # Test if we can access it without ElementLostError
                _ = iframe.attrs
                break
            except Exception:
                time.sleep(0.5)
        ...
        if not iframe:
            raise Exception("Challenge iframe not found or lost")
    ...
    def is_detected(self) -> bool:
        ...
        try:
            iframe = self.driver("xpath://iframe[contains(@title, 'recaptcha challenge')]")
            return (
                iframe.ele("Try again later", timeout=self.TIMEOUT_DETECTION)
                .states()
                .is_displayed
            )
        except Exception:
