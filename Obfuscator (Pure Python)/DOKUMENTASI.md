...
sekaligus menyajikan halaman web. Jalankan:
    ...
    python3 app.py             # buka http://127.0.0.1:8000/
...
- `POST /api/stego/encode` — sisipkan pesan ke bit LSB gambar PNG (upload file).
- `POST /api/stego/decode` — ekstrak pesan LSB dari gambar PNG.
- `POST /api/jwt/decode`   — baca header/payload/klaim JWT + verifikasi signature (HMAC).
- `POST /api/jwt/encode`   — buat & tanda tangani token JWT baru (HS256/384/512).
...
## 2. Struktur Direktori
    ...
    ├── obfuscate.py          # CLI non-interaktif
    ├── stego_umum.py         # CLI ekstraktor pesan LSB pada PNG
    ├── jwt_umum.py           # CLI JWT (decode/verify/encode)
...
### `stego.py`
Steganografi LSB pada PNG, murni stdlib (`struct` + `zlib`, tanpa Pillow).
- `read_png_rgba(data)` / `write_png_rgba(w, h, rgba)` — baca/tulis PNG ke/dari RGBA.
- `encode(png_bytes, message)` — sisipkan pesan pada bit LSB channel R,G,B.
- `decode(png_bytes)` — rangkai kembali pesan dari bit LSB.
- `readability(text)` — skor keterbacaan untuk menandai hasil yang bukan teks.

### `jwt_tool.py`
Perkakas JWT murni stdlib (`hmac` + `hashlib` + `base64` + `json`), HMAC HS256/384/512.
- `decode(token)` — pisah 3 bagian, kembalikan header, payload, signature, algoritma,
  dan `claims` (klaim standar exp/iat/nbf ditafsirkan jadi waktu + status berlaku).
- `verify(token, secret)` — hitung ulang HMAC lalu bandingkan konstan-waktu
  (`hmac.compare_digest`); kembalikan `{verified, algorithm, reason}`.
- `encode(payload, secret, algorithm, header)` — susun & tanda tangani token baru.
- `b64url_encode/decode` — base64url tanpa padding sesuai spesifikasi JWT.
